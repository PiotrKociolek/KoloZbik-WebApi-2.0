...
const ActiveMembers = () => {
    ...
    // Fetch active members data from API
    useEffect(() => {
        const fetchMembers = async () => {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/UserLists/Get-UserList`);
                                ...
                                <div key={member.id} className="bg-gray-900 p-4 rounded-lg text-center">
                                    <div className="flex items-center justify-center space-x-2">
                                        ...
                                        <b className="text-lg font-semibold text-white">
                                            {member.name} {member.lastName} {/* Changed surname to lastName */}
                                        </b>
                                    </div>
                                </div>
                            ))}
